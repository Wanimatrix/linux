...
static int lm3533_pdata_from_of_node(struct device *dev)
{
	...
	const unsigned ovps[] = {
		[LM3533_BOOST_OVP_16V] = 16000000,
		[LM3533_BOOST_OVP_24V] = 24000000,
		[LM3533_BOOST_OVP_32V] = 32000000,
		[LM3533_BOOST_OVP_40V] = 40000000,
	};
	...
	ret = lm3533_of_parse_enum(dev->of_node, "ti,boost-ovp-microvolt", ovps, 4);
	if (ret < 0) {
		dev_err(dev, "failed to parse ti,boost-ovp-microvolt\n");
		return ret;
	}
	pdata->boost_ovp = ret;
